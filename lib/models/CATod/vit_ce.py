...
from .attn_blocks import CEBlock_cat
...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.point_pos_embed = nn.Parameter(torch.zeros(1, 49, 768))
        self.pos_embed_Adapter = nn.Parameter(torch.zeros(1, 576, 768))

    def forward_features(self, x, prompt_feat, pro_id, mask_z=None, mask_x=None, ce_template_mask=None,
                         ce_keep_rate=None, return_last_attn=False, track_query=None, token_type="add",
                         token_len=1, s=0):
        ...
        x = x + self.pos_embed_Adapter

        lens_z = 49
        lens_x = x.shape[1]  # HW

        x = combine_tokens(prompt_feat+self.point_pos_embed, x, mode=self.cat_mode)  # (B, z+x, 768)
        if self.add_cls_token:
            ...
            if self.add_cls_token:
                x, global_index_t, global_index_s, removed_index_s, attn = \
                    blk(x, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate,
                        add_cls_token=self.add_cls_token, query_len=query_len, s=1)
            else:
                x, global_index_t, global_index_s, removed_index_s, attn = \
    ...
    def forward(self, z, x, ce_template_mask=None, ce_keep_rate=None,
                ...
                token_type="add", token_len=1, prompt=None, pro_id=None, s=0):
        x, aux_dict = self.forward_features(x, prompt, pro_id, ce_template_mask=ce_template_mask,
               ce_keep_rate=ce_keep_rate, track_query=track_query, token_type=token_type, token_len=token_len,s=s)
        return x, aux_dict


    def stage2(self, z=None, x=None, prompt_feat=None, mask_z=None, mask_x=None,
                         ce_template_mask=None, ce_keep_rate=None,
                         return_last_attn=False, track_query=None,
                         token_type="add", token_len=1, s=0, mode=None
                         ):
        B, H, W = x.shape[0], x.shape[2], x.shape[3]
        ...
        if self.add_cls_token:
            x = torch.cat([query, prompt_feat+self.point_pos_embed, x], dim=1)     # (B, 1+z+x, 768)
            query_len = query.size(1) + 49
        x = self.pos_drop(x)
        ...
        for i, blk in enumerate(self.blocks):
            if self.add_cls_token:
                x, global_index_t, global_index_s, removed_index_s, attn = \
                    blk(x, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate,
                        add_cls_token=self.add_cls_token, query_len=query_len, s=2)
            else:
                x, global_index_t, global_index_s, removed_index_s, attn = \
                    blk(x, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate, add_cls_token=self.add_cls_token)
    ...
    def forward_track(self, z, x, mask_z=None, mask_x=None,
                         ce_template_mask=None, ce_keep_rate=None,
                         return_last_attn=False, track_query=None,
        ...
        for i, blk in enumerate(self.blocks):
            if self.add_cls_token:
                x, global_index_t, global_index_s, removed_index_s, attn = \
                    blk(x, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate,
                        add_cls_token=self.add_cls_token, query_len=query_len, mode='track')
            else:
                x, global_index_t, global_index_s, removed_index_s, attn = \
                    blk(x, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate,
                        add_cls_token=self.add_cls_token, mode='track')
