...
def transform_image_to_crop(box_in: torch.Tensor, box_extract: torch.Tensor, resize_factor: float,
    ...
    box_out = torch.cat((box_out_center - 0.5 * box_out_wh, box_out_wh))
    if normalize:
        return box_out / crop_sz[0]  #, box_out
    else:
        return box_out

def box_ex_256(box_in: torch.Tensor, box_extract: torch.Tensor, resize_factor: float,
                            crop_sz: torch.Tensor, normalize=False) -> torch.Tensor:
    box_ex_256_center = (crop_sz - 1) / 2
    box_ex_256_wh = box_extract[2:4] * resize_factor
    box_ex_256 = torch.cat((box_ex_256_center, box_ex_256_wh))
    return box_ex_256 / crop_sz[0]  # , box_out
...
def jittered_center_crop(frames, box_extract, box_gt, search_area_factor, output_sz, masks=None):
    ...
    box_extract_256 = [box_ex_256(a_gt, a_ex, rf, crop_sz, normalize=True)
                for a_gt, a_ex, rf in zip(box_gt, box_extract, resize_factors)]

    return frames_crop, box_crop, att_mask, masks_crop, box_extract_256
